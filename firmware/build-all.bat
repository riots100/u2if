...
rem for /f "tokens=2 delims== " %%a in ('findstr "project(u2if VERSION" "%CD%\source\CMakeLists.txt"') do set "VERSION=%%a"

set "RELEASE_DIR=%CD%\release"

REM Get the number of processors
rem for /f %%a in ('wmic cpu get NumberOfLogicalProcessors /value') do (
rem     for /f "delims=" %%b in ("%%a") do set "CORES_NB=%%b"
rem )
rem set "CORES_NB=%CORES_NB:NumberOfLogicalProcessors=%%"
...
:build
    ...
    ninja 
    copy u2if.uf2 "%RELEASE_DIR%\u2if_%1.uf2"
